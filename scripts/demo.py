...
def fusion_callback(event):
    ...
    if display_vi_fusion:
        win_vi_fusion = np.ones((win_h, win_w * 3, 3), dtype=np.uint8) * 255
        win_vi_fusion[:, :win_w] = current_image1.copy()
        win_vi_fusion[:, win_w * 2:] = current_image2.copy()
        image_vi = vi.fusion(arr_visible=image1_frame, arr_infrared=image2_frame).clip(0.0, 255.0)
        image_vi = draw_object_model_from_main_view(
            image_vi, objs, frame, display_frame,
            display_obj_state, thickness=2
        )
        win_vi_fusion[:, win_w:win_w * 2] = image_vi
    ...
    if display_vi_fusion:
        cv2.namedWindow("vi_fusion", cv2.WINDOW_NORMAL)
        cv2.imshow("vi_fusion", win_vi_fusion)
        if developer_mode: v_vi_fusion.write(win_vi_fusion)
    
    # 显示窗口时按Esc键终止程序
    display = [display_image_raw, display_image_segmented,
        display_2d_modeling, display_3d_modeling, display_vi_fusion].count(True) > 0
    if display and cv2.waitKey(1) == 27:
        ...
        if display_vi_fusion:
            cv2.destroyWindow("vi_fusion")
            if developer_mode:
                v_vi_fusion.release()
            print("Save video of vi_fusion.")
...
if __name__ == '__main__':
    ...
    display_vi_fusion = rospy.get_param("~display_vi_fusion")
    ...
    if display_3d_modeling and developer_mode:
        ...
        v_image2_3d_modeling = cv2.VideoWriter(v_path, v_format, frame_rate,
            (win_w, win_h), True)
    if display_vi_fusion and developer_mode:
        v_path = 'vi_fusion.mp4'
        v_format = cv2.VideoWriter_fourcc(*"mp4v")
        v_vi_fusion = cv2.VideoWriter(v_path, v_format, frame_rate,
            (win_w * 3, win_h), True)
    
    if display_vi_fusion:
        from vi_fusion import VIFusion
        vi = VIFusion()
