...
while cwd[idx] != '/':
    idx -= 1
sys.path.append(cwd[:idx] + '/modules/yolact-test')
...
class YolactDetector():
    def __init__(self, model='/weights/seumm_lwir/yolact_resnet50_72_60000.pth'):
        ...
        # Yolact参数配置
        trained_model = cwd[:idx] + '/modules/yolact-test' + model
        pth = SavePath.from_str(trained_model)
        config = pth.model_name + '_config'
        set_cfg(config)
        set_dataset('seumm_lwir_dataset')
...
if __name__ == '__main__':
    img = cv2.imread('/home/lishangjie/data/SEUMM/seumm_lwir/images/001000.jpg')
